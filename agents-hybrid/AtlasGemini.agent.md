...
### Implementation Agents (2-tier coding)
...
2. **Odysseus-subagent** (cheap): THE WORKHORSE ENGINEER. Handles ALL standard implementation from simple single-file changes to multi-file features. TDD. Best for: 1-4 files, CRUD, API endpoints, standard patterns, integration, UI features, config changes, boilerplate, simple fixes.
...
| 10+ line implementation | Maybe if trivial | ✅ Odysseus |
| Multi-file feature (3+ files) | ❌ | ✅ Odysseus (standard) / Daedalus (complex) |
...
1. **Choose approach based on cost/benefit:**
   ...
   - Anything larger: Delegate to Odysseus (standard) or Daedalus (complex architecture, 5+ files).
   - When in doubt, use Odysseus. Reserve Daedalus for genuinely complex work.

2. **Parallelization:** Invoke multiple Odysseus instances for independent tasks within the same phase.
